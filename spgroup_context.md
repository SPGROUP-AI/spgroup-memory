# SPGROUP — 전체 시스템 컨텍스트
Last Updated: 2026-05-04 (자동)

## 회사 정보
- 회사명: SP GROUP CO., LTD (주식회사 에스피그룹)
- 업종: 가먼트(의류) 수출 벤더
- 운영 시장: KR / US / 설립: 2022.12
- 홈페이지: https://spgpinc.com

## 핵심 연락처
- Brian Lee (Director): brianlee@spgpinc.com
- JARVIS (AI): jarvis@spgpinc.com

## JARVIS 시스템
- Sheets DB: 1gQ2VOsA2Eo7eUdgShj9BxpnWGj99QddpG5rV2ltmDGk
- GitHub Website: SPGROUP-AI/spgroup-website
- GitHub Memory: SPGROUP-AI/spgroup-memory
- AutoReply 태그: [SPGROUP INQUIRY]
- 알림 기준: URGENT만 즉시 알림

## 트리거 5개 (jarvis@spgpinc.com)
- onNewEmailReceived: 5분마다
- checkAndAutoReply: 5분마다
- generateDailyReport: 매일 09:00 (월~토)
- checkUnrepliedEmails: 매일 14:00
- generateWeeklyReport: 매주 월 08:00

## 최근 7일 이메일 통계 (자동)
- 총: 0건 / URGENT: 0 / HIGH: 0 / NORMAL: 0

## 에이전트 팀
| 에이전트 | 상태 | memory |
|---------|------|--------|
| JARVIS | 자동화 가동 | agents/jarvis_memory.md |
| HUNTER | 대화형 | agents/hunter_memory.md |
| APEX | 대화형 | agents/apex_memory.md |
| VERA | 대화형 | agents/vera_memory.md |
| LUMINA | 자동화 가동 | agents/lumina_memory.md |

## VERA 컬러
Midnight #1A1F2E / SP Blue #2D4FC4 / Periwinkle #6B8CFF
SP Taupe #9B8B72 / Warm Grey #E8E4DD / Chalk #FAFAF8

자동 업데이트: 2026-05-04
